...
There are many advantages of using Service Meshes. One of the greatest is the increased visibility they can provide.  This post will demonstrate how to view the envoy statistics available to App Mesh. These [envoy statistics](https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/observability/statistics#arch-overview-statistics) are automatically generated in order for a Prometheus scraper to gather them.  It is also quite easy to turn on [statsD](https://github.com/statsd/statsd/wiki) exporting within the envoy proxy.  The advantage of statsD, is that this format is easily consumed by a [CloudWatch agent](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Install-CloudWatch-Agent.html). This agent can then forward the stats to CloudWatch, allowing for dashboards, alarms and so on. This approach is demonstrated by the [2019 ReInvent talk - CON328, Improving observability of your containers](https://www.youtube.com/embed/O1NQrIm_4cg).
